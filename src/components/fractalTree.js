...
const FractalTree = () => {
  ...
  // NOTE: We'll only read the canvas size and do the transforms ONCE in setup
  // so that scrolling does NOT distort the drawing.
...
`;
        ...
        const sketch = (p5) => {
          // We'll read the canvas size only ONCE in setup
          let canvasWidth;
          let canvasHeight;

          // We'll store epicycle data
          let githubFourierX = [];
          let githubFourierY = [];
          ...
          // We'll keep only the initial canvas size here, no recalculation
          let originalWidth = 0;
          let originalHeight = 0;

          // We'll store original data
          ...
          p5.setup = function() {
            ...
            // Make sure we only handle events inside canvas
            ...
            if (pathTags.length === 0) {
              console.error('No <path> elements found in SVG data.');
              return;
            }
            ...
            if (allPathSketches.length === 0 || !allPathSketches[0].length) {
              console.error('SVG path is empty or undefined.');
              return;
            }
            ...
            // Center offset for GitHub shape, only once
          ...
          // We do NOT override windowResized or recalc offsets. 
          // We let the canvas scroll, 
          // but do not recenter or recalc the dimension on scroll.
          ...
          // We do NOT recenter in "windowResized" at all, 
          // so no additional logic is placed here.
          ...
          p5.draw = function() {
            ...
            if (!userStartedDrawing && githubFourierX.length > 0 && githubFourierY.length > 0) {
              const vx = epiCycles(p5, githubTime, githubOffset.x, githubOffset.y, 0, githubFourierX);
              const vy = epiCycles(p5, githubTime, githubOffset.x, githubOffset.y, p5.HALF_PI, githubFourierY);
