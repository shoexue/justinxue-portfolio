...
const FractalTree = () => {
  ...
  useEffect(() => {
    // Dynamically import p5 to ensure it's only loaded on the client side
    import('p5')
      .then(p5Module => {
...
`;
        ...
        const sketch = p5 => {
          ...
          p5.setup = function() {
            ...
            cnv.style('touch-action', 'none'); // Prevent default touch actions
            p5.frameRate(60);
            p5.background(0);

            canvasElement = cnv.elt; // Store the canvas DOM element
          ...
          // Helper function to get relative mouse/touch positions
